...
char *get_backup_name(const char *fname)
{
	...
	rprintf(FERROR, "ERR : backup filename too long\n");
	return NULL;
}
...
static int make_simple_backup(const char *fname)
{
	...
	while (1) {
		if (do_rename(fname, fnamebak) == 0) {
			if (verbose > 1) {
				rprintf(FINFO, "INF : backed up %s to %s\n",
					fname, fnamebak);
			}
			break;
		}
...
static int keep_backup(const char *fname)
{
	...
	if ((am_root && preserve_devices && IS_DEVICE(file->mode))
	 || (preserve_specials && IS_SPECIAL(file->mode))) {
		...
		if (verbose > 2 && save_errno == 0) {
			rprintf(FINFO, "INF : make_backup: DEVICE %s successful.\n",
				fname);
		}
		kept = 1;
		do_unlink(fname);
	...
	if (!kept && S_ISDIR(file->mode)) {
		...
		if (verbose > 2) {
			rprintf(FINFO, "INF : make_backup: RMDIR %s returns %i\n",
	...
	if (!kept && preserve_links && S_ISLNK(file->mode)) {
		...
		if (safe_symlinks && unsafe_symlink(sl, fname)) {
			if (verbose) {
				rprintf(FINFO, "INF : ignoring unsafe symlink %s -> %s\n",
					full_fname(buf), sl);
			}
			kept = 1;
		} else {
	...
	if (!kept && !S_ISREG(file->mode)) {
		rprintf(FINFO, "INF : make_bak: skipping non-regular file %s\n",
			fname);
		unmake_file(file);
#ifdef SUPPORT_ACLS
		uncache_tmp_acls();
