	...
	for (rc = sys_acl_get_entry(sacl, SMB_ACL_FIRST_ENTRY, &entry);
		...
		case SMB_ACL_USER_OBJ:
			if (racl->user_obj == NO_ENTRY)
				...
				rprintf(FINFO, "INF : unpack_smb_acl: warning: duplicate USER_OBJ entry ignored\n");
			continue;
		case SMB_ACL_GROUP_OBJ:
			if (racl->group_obj == NO_ENTRY)
				racl->group_obj = access;
			else
				rprintf(FINFO, "INF : unpack_smb_acl: warning: duplicate GROUP_OBJ entry ignored\n");
			continue;
		case SMB_ACL_MASK:
			if (racl->mask_obj == NO_ENTRY)
				racl->mask_obj = access;
			else
				rprintf(FINFO, "INF : unpack_smb_acl: warning: duplicate MASK entry ignored\n");
			continue;
		case SMB_ACL_OTHER:
			if (racl->other_obj == NO_ENTRY)
				racl->other_obj = access;
			else
				rprintf(FINFO, "INF : unpack_smb_acl: warning: duplicate OTHER entry ignored\n");
			continue;
#endif
		case SMB_ACL_USER:
			access |= NAME_IS_USER;
		...
		case SMB_ACL_GROUP:
			break;
		default:
			rprintf(FINFO, "INF : unpack_smb_acl: warning: entry with unrecognized tag type ignored\n");
			continue;
		}
...
static BOOL pack_smb_acl(SMB_ACL_T *smb_acl, const rsync_acl *racl)
{
	...
	if (sys_acl_valid(*smb_acl) < 0)
		rprintf(FERROR_XFER, "ERR : pack_smb_acl: warning: system says the ACL I packed is invalid\n");
#endif
...
static uint32 recv_acl_access(uchar *name_follows_ptr, int f)
{
	...
	if (name_follows_ptr) {
	  ...
	  value_error:
		rprintf(FERROR_XFER, "ERR : recv_acl_access: value out of range: %x\n",
			access);
...
static int recv_rsync_acl(item_list *racl_list, SMB_ACL_TYPE_T type, int f)
{
	...
	if (ndx < 0 || (size_t)ndx > racl_list->count) {
		rprintf(FERROR_XFER, "ERR : recv_acl_index: %s ACL index %d > %d\n",
...
int default_perms_for_dir(const char *dir)
{
	...
	if (sacl == NULL) {
		...
		default:
			rprintf(FWARNING,
				"WRM : default_perms_for_dir: sys_acl_get_file(%s, %s): %s, falling back on umask\n",
	...
	sys_acl_free_acl(sacl);
	if (!ok) {
		rprintf(FWARNING, "WRM : default_perms_for_dir: unpack_smb_acl failed, falling back on umask\n");
		return perms;
	}
	...
	if (racl.user_obj != NO_ENTRY) {
		...
		if (verbose > 2)
			rprintf(FINFO, "INF : got ACL-based default perms %o for directory %s\n", perms, dir);
	}
