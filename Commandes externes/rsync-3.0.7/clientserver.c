			...
			int argc, char *argv[])
{
	...
	if (**remote_argv == '/') {
		rprintf(FERROR,
			"ERR : The remote path must start with a module name not a /\n");
		return -1;
	}
...
static int exchange_protocols(int f_in, int f_out, char *buf, size_t bufsiz, int am_client)
{
	...
	if (!read_line_old(f_in, buf, bufsiz)) {
		if (am_client)
			rprintf(FERROR, "ERR : rsync: did not see server greeting\n");
		return -1;
	}

	if (sscanf(buf, "@RSYNCD: %d.%d", &remote_protocol, &remote_sub) < 1) {
		if (am_client)
			rprintf(FERROR, "ERR : rsync: server sent \"%s\" rather than greeting\n", buf);
		else
	...
	if (remote_sub < 0) {
		if (remote_protocol == 30) {
			if (am_client)
				rprintf(FERROR, "ERR : rsync: server is speaking an incompatible beta of protocol 30\n");
			else
...
int start_inband_exchange(int f_in, int f_out, const char *user, int argc, char *argv[])
{
	...
	if (**argv == '/') {
		rprintf(FERROR,
			"ERR : The remote path must start with a module name\n");
		return -1;
	}
	...
	while (argc > 0) {
		if (sargc >= MAX_ARGS - 1) {
		  arg_overflow:
			rprintf(FERROR, "ERR : internal: args[] overflowed in do_cmd()\n");
	...
	while (1) {
		if (!read_line_old(f_in, line, sizeof line)) {
			rprintf(FERROR, "ERR : rsync: didn't get server startup line\n");
			return -1;
		}
...
static int path_failure(int f_out, const char *dir, BOOL was_chdir)
{
	if (was_chdir)
		...
		rprintf(FLOG, "ERR : normalize_path(%s) failed\n", dir);
...
static int rsync_module(int f_in, int f_out, int i, char *addr, char *host)
{
	...
	if (!allow_access(addr, host, lp_hosts_allow(i), lp_hosts_deny(i))) {
		rprintf(FLOG, "ERR : rsync denied on module %s from %s (%s)\n",
			name, host, addr);
		if (!lp_list(i))
	...
	if (am_daemon && am_server) {
		rprintf(FLOG, "ERR : rsync allowed access on module %s from %s (%s)\n",
			name, host, addr);
	}
	...
	if (!claim_connection(lp_lock_file(i), lp_max_connections(i))) {
		if (errno) {
			...
			rprintf(FLOG, "ERR : max connections (%d) reached\n",
	...
	if (am_root) {
		...
		if (!name_to_uid(p, &uid)) {
			if (!isDigit(p)) {
				rprintf(FLOG, "LOG : Invalid uid %s\n", p);
		...
		if (!name_to_gid(p, &gid)) {
			if (!isDigit(p)) {
				rprintf(FLOG, "LOG : Invalid gid %s\n", p);
	...
	if (munge_symlinks) {
		...
		if (do_stat(SYMLINK_PREFIX, &st) == 0 && S_ISDIR(st.st_mode)) {
			rprintf(FLOG, "LOG : Symlink munging is unsupported when a %s directory exists.\n",
				SYMLINK_PREFIX);
	...
	if (lp_temp_dir(i) && *lp_temp_dir(i)) {
		...
		if (strlen(tmpdir) >= MAXPATHLEN - 10) {
			rprintf(FLOG,
				"LOG : the 'temp dir' value for %s is WAY too long -- ignoring.\n",
				name);
			tmpdir = NULL;
		}
	}
	...
	if (request) {
		if (*auth_user) {
			rprintf(FLOG, "ERR : rsync %s %s from %s@%s (%s)\n",
				am_sender ? "on" : "to",
				request, auth_user, host, addr);
		} else {
			rprintf(FLOG, "ERR : rsync %s %s from %s (%s)\n",
				am_sender ? "on" : "to",
				request, host, addr);
		}
		free(request);
	...
	if (*p && !(daemon_chmod_modes = parse_chmod(p, &chmod_modes))) {
		rprintf(FLOG, "LOG : Invalid \"%sing chmod\" directive: %s\n",
...
int start_daemon(int f_in, int f_out)
{
	...
	rprintf(FLOG, "LOG : connect from %s (%s)\n", host, addr);
	...
	if (!*line || strcmp(line, "#list") == 0) {
		rprintf(FLOG, "LOG : module-list request from %s (%s)\n",
			host, addr);
		send_listing(f_out);
		return -1;
	}
	...
	if ((i = lp_number(line)) < 0) {
		rprintf(FLOG, "LOG : unknown module '%s' tried from %s (%s)\n",
			line, host, addr);
		io_printf(f_out, "@ERROR: Unknown module '%s'\n", line);
		return -1;
	}
...
static void create_pid_file(void)
{
	...
	if ((fd = do_open(pid_file, O_WRONLY|O_CREAT|O_EXCL, 0666 & ~orig_umask)) == -1) {
	  failure:
		cleanup_set_pid(0);
		fprintf(stderr, "ERR : failed to create pid file %s: %s\n", pid_file, strerror(errno));
...
static void become_daemon(void)
{
	...
	if (pid) {
		if (pid < 0) {
			fprintf(stderr, "ERR : failed to fork: %s\n", strerror(errno));
			exit_cleanup(RERR_FILEIO);
		}
		_exit(0);
	}
...
int daemon_main(void)
{
	...
	if (!load_config(1)) {
		fprintf(stderr, "ERR : Failed to parse config file: %s\n", config_file);
		exit_cleanup(RERR_SYNTAX);
	}
	...
	rprintf(FLOG, "ERR : rsyncd version %s starting, listening on port %d\n",
