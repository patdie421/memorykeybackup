...
char *client_name(int fd)
{
	...
	if (am_server) {	/* daemon over --rsh mode */
		...
		if ((err = getaddrinfo(addr, NULL, &hint, &answer)) != 0) {
			rprintf(FLOG, "LOG : malformed address %s: %s\n",
		...
		char *port_buf, size_t port_buf_size)
{
	...
	if (name_err != 0) {
		...
		rprintf(FLOG, "LOG : name lookup failed for %s: %s\n",
			      ...
			      const struct sockaddr_storage *ss)
{
	...
	if (ai->ai_family != ss_family) {
		rprintf(FLOG, "LOG : %s: response family %d != %d\n",
	...
	if (ss_family == AF_INET6) {
		...
		if (ai->ai_addrlen < sizeof (struct sockaddr_in6)) {
			rprintf(FLOG, "LOG : %s: too short sockaddr_in6; length=%d\n",
	       ...
	       char *name_buf, size_t name_buf_size)
{
	...
	if (error) {
		rprintf(FLOG, "LOG : forward name lookup for %s failed: %s\n",
	...
	if (!res0) {
		...
		rprintf(FLOG, "LOG : no known address for \"%s\": "
			"spoofed address?\n", name_buf);
		strlcpy(name_buf, default_name, name_buf_size);
	} else if (res == NULL) {
		/* We hit the end of the list without finding an
		 * address that was the same as ss. */
		rprintf(FLOG, "LOG : %s is not a known address for \"%s\": "
